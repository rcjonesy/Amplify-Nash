            ...
            <div className="mb-6">
              ...
              <div className="mb-3">
                <p className="text-lg mb-2">
                  <span className="font-semibold text-xl text-blue-500">Genre:</span>
                  <span className="ml-2 text-xl font-bold">{band.genre}</span>
                </p>
              </div>
              {/* Bio */}
              <div className="mb-3">
                <p className="text-lg mb-2"><span className="font-semibold text-xl text-blue-500">Bio:</span></p>
                <p className="text-lg text-gray-800">{band.bio}</p>
              </div>
              {/* Band Members */}
              {band.bandMembers && band.bandMembers.length > 0 ? (
                <div className="mb-4">
                  <p className="text-lg mb-2"><span className="font-semibold text-xl text-blue-500">Band Members:</span></p>
                  <ul className="list-disc pl-6">
                    {band.bandMembers.map((member) => (
                      <li key={member.id} className="mb-2">
                        <p className="text-lg font-semibold">{member.name}</p>
                        <p className="text-md text-gray-800"><span className="font-semibold">Instrument: </span>{member.instrument}</p>
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <ul className="list-disc pl-6">
                    {band.bandConcerts.map((bandConcert) => (
                      <li key={bandConcert.id} className="mb-2">
                        <p className="text-lg font-semibold">{bandConcert.concert.venue.name}</p>
                        <p className="text-md text-gray-800"><span className="font-semibold">Date: </span>{new Date(bandConcert.concert.date).toLocaleDateString()}</p>
                      </li>
                    ))}
                  </ul>
                ) : (
