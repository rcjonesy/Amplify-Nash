...
export const Home = ({ loggedInUser }) => {
  ...
  useEffect(() => {
    ...
    const foundConcerts = concerts?.filter((concert) => {
      ...
      const venueMatches = concert?.venue?.name
        .toLowerCase()
        .includes(searchTerm);
                  ...
                  <td className="px-6 py-4 text-center text-lg">
                    <ul className="pl-0 list-none flex flex-col items-center">
                      {concert.bandConcerts.length > 0 ? (
                        ...
                        <li>No supporting Artists</li>
                      )}
                    </ul>
                  </td>
                  <td className="text-center align-middle ">
                    {concert.bandConcerts.some((bc) => bc.band.isHeadliner) &&
                    concert.bandConcerts.filter((bc) => !bc.band.isHeadliner)
                      .length >= 2 ? (
                      <span className=" text-green-500 text-3xl inline-block">
                        <FcCheckmark />
                      </span>
                    ) : (
                      <span className="text-red-500 text-3xl inline-block">
                        <FcHighPriority />
                      </span>
                    )}
                  </td>
